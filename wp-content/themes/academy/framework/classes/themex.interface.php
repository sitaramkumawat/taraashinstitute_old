...
class ThemexInterface {
	...
	public static function renderDropdownMenu($slug) {
		...
		if(isset($locations[$slug])) {
			...
			if(isset($menu->term_id)) {
				...
				$menu_ids=wp_list_pluck($menu_items, 'ID');				
				$menu_items=array_combine($menu_ids, $menu_items);
				...
				if($menu_items!==false) {
					foreach ($menu_items as $menu_item) {
						...
						if(!empty($menu_item->menu_item_parent)) {
							...
							while(isset($menu_items[$parent])) {
								$title='&nbsp;&nbsp;&nbsp;'.$title;
								$parent=$menu_items[$parent]->menu_item_parent;
							}
						}
						
						$out.='<option value="'.$menu_item->url.'">'.$title.'</option>';
					}
				}				
				$out.='</select>';
	...
	public static function renderCommentForm($args) {
		...
		if(is_singular('lesson')) {
			$args['comment_field']='<div class="formatted-form"><div class="field-wrapper"><input id="title" name="title" type="text" placeholder="'.__('Question', 'academy').'" /></div>';
			$args['comment_field'].='<div class="field-wrapper"><textarea id="comment" name="comment" cols="45" rows="8" placeholder="'.__('Comment', 'academy').'"></textarea></div></div>';
			$args['comment_notes_before']='';
			$args['comment_notes_after']='';
			$args['logged_in_as']='';
			$args['label_submit']=__('Add Question', 'academy');
		} else {
			$args['logged_in_as']='<div class="formatted-form">';
			$args['comment_notes_before']='<div class="formatted-form">';
			$args['comment_notes_after']='</div>';
			$args['fields']['author']='<div class="field-wrapper"><input id="author" name="author" type="text" value="" size="30" placeholder="'.__('Name', 'academy').'" /></div>';
			$args['fields']['email']='<div class="field-wrapper"><input id="email" name="email" type="text" value="" size="30" placeholder="'.__('Email', 'academy').'" /></div>';
			$args['fields']['url']='';
			$args['comment_field']='<div class="field-wrapper"><textarea id="comment" name="comment" cols="45" rows="8" placeholder="'.__('Comment', 'academy').'"></textarea></div>';
			$args['label_submit']=__('Add Comment', 'academy');
		}	
